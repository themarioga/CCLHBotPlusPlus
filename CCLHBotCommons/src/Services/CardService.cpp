...
Player CardService::GetPlayerFromRoundWhiteCardID(int64_t game_id, int64_t card_id) {
	try {
		//Query
		std::shared_ptr<SQLite::Statement> query = Db::Instance()->CreateQuery(
			"SELECT users.id, users.name, users.active, "
			...
			"AND playersxgamesxcards_roundwhitecards.game_id = ?"
		);
		query->bind(1, card_id);
		query->bind(2, game_id);
...
void CardService::DeleteAllCards(int64_t dictionary_id) {
	try {
		std::shared_ptr<SQLite::Statement> query = Db::Instance()->CreateQuery("DELETE FROM cards WHERE dictionary_id=?");
...
uint64_t CardService::CountCards(int8_t card_type, int64_t dictionary_id) {
	try {
		std::string sql = "SELECT COUNT(*) FROM cards WHERE text IS NOT NULL";
		if (card_type != -1) sql += " AND type = "+std::to_string(card_type);
		if (dictionary_id != -1) sql += " AND dictionary_id = "+std::to_string(dictionary_id);
...
std::vector<Card> CardService::GetAllCards(int8_t card_type, int64_t dictionary_id) {
	try {
		std::vector<Card> cards;

		std::string sql = "SELECT id, type, text, dictionary_id FROM cards WHERE text IS NOT NULL";
		if (card_type != -1) sql += " AND type = "+std::to_string(card_type);
		if (dictionary_id != -1) sql += " AND dictionary_id = "+std::to_string(dictionary_id);
